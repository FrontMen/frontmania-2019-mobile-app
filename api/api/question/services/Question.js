...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Question.associations
  ...
  add: async (values) => {
    ...
    return Question.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Question.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Question.attributes).reduce((acc, curr) => {
      switch (Question.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
